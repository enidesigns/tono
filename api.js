const API = {
  ...
  async call(prompt, maxTokens = 1500) {
    ...
    if (!res.ok) {
      ...
      if (res.status >= 500) throw Object.assign(new Error('Server error'), { code: 'SERVER_ERROR' });
      throw Object.assign(new Error(body.error?.message || 'API error'), { code: 'API_ERROR' });
    }
